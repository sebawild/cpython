 ...
 * with 2**64 elements.
*/
// NEW #define MAX_MERGE_PENDING (SIZEOF_SIZE_T * 8)
...
struct s_slice {
    ...
    // NEW int power; /* node "level" for powersort merge strategy */
};
...
struct s_MergeState {
    ...
    //NEW Py_ssize_t listlen;     /* len(input_list) - read only */
    //NEW PyObject **basekeys;    /* base address of keys array - read only */
...
//NEW merge_init(MergeState *ms, Py_ssize_t list_size, int has_keyfunc,
           //sortslice *lo)
{
    assert(ms != NULL);
    if (has_keyfunc) {
    ...
    //NEW ms->listlen = list_size;
    //NEW ms->basekeys = lo->keys;
}
